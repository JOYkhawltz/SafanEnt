@extends('layouts.app', ['page' => 'New Sector', 'pageSlug' => 'sector', 'section' => 'management'])
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">New Sector</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('sectors.index') }}" class="btn btn-sm btn-primary">Back to List</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('sectors.store') }}" autocomplete="off">
                            @csrf

                            <h6 class="heading-small text-muted mb-4">Sector Information</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-name">Sector Name</label>
                                    <input type="text" name="sector_name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="Name" value="{{ old('name') }}" required autofocus>
                                    @include('alerts.feedback', ['field' => 'sector_name'])
                                </div>
